    ...
    public class UserController : BaseController
    {
        ...
        public ActionResult Create(int? Id)
        {
            ...
            if ((UserRoleEnum)Enum.Parse(typeof(UserRoleEnum), Session["USERROLE"].ToString()) == UserRoleEnum.Admin)
            {
                ...
                IEnumerable<UserRoleDTO> userRole = usermanager.ViewtUserRole();

                //Map and UserRoleDTO object to UserRoleModel and put into  UserModel.UserRoles list
                userModel.UserRoles = userRole.Select(m => new UserRoleModel
                {
                    Id = m.Id,
                    Role = m.Role
                }).ToList();

                //return user role data to view
                ViewBag.UserRoleId = new SelectList(userRole, "Id", "Role");
                return View();
            }
            //Redirect to error page if user role dont have access to this function
            else
        ...
        public ActionResult Edit(int? id)
        {
            ...
            if ((UserRoleEnum)Enum.Parse(typeof(UserRoleEnum), Session["USERROLE"].ToString()) == UserRoleEnum.Admin)
            {
                ...
                //return user role list data to view
                ViewBag.UserRoleId = new SelectList(userRole, "Id", "Role", usermodel.UserRoleId);
                return View(usermodel);
            }
            //Redirect to error page if user role dont have access to this function
            else
